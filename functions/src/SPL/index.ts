    ...
    .once('value', snapshot => {
      ...
      if (splData === null || splData === undefined) {
        res
          .status(404)
          .send(
            'Error: There is no SPL data associated with this bulletin in the database.'
          );
      }
  ...
  const bulletinSplPromise = aospVerToBulletinPromise.then(snapshot => {
    ...
    if (bulletinData === null || bulletinData === undefined) {
      res
        .status(404)
        .send(
          'Error: There are no SPLs associated with this Android Version ID in the database.'
        );
    }
    const promises = [];
