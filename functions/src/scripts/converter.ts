...
function postCVEHistory(data: any, tree: any, version_number: string): Promise<any> {
    ...
    return ref.once('value').then(function (snapshot) {
        ...
        return buildCVEHistoryTree(result, version_number, tree, data);
    }).catch(error => { console.log("Error with writing new CVE history tree to db: " + error) });
}


function buildCVEHistoryTree(tree: any, versionHistory: string, cveTree: any, jsonData: any): any {
    //key is ASB:Version
...
function writeToDatabaseCVETree(data: any, versionNum: string, longNum: any): Promise<any> {
    ...
    const currentTreeCheck = currentTree.then((snapshot) => {
        ...
        if (result === null) {
            ...
            versionOK = validVersionNumber(versionNum, result, data);
        }
        const promises = [];
...
function validVersionNumber(versionNum: string, tree: any, JSONData: any): boolean {
...
function buildBulletinSPLTree(tree: any): Map<string, Array<any>> {
...
function buildSPLCVEIDTree(tree: any): Map<any, any> {
...
function buildAOSPVersionASBCVEIDTree(tree: any): Map<any, any> {
