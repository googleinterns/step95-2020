...
export async function convert(bulletinData: any, versionInput: any) {
    const rawCVE = getCVEs(bulletinData, versionInput.toString()); //create CVE tree
    ...
    if (JSONData) {
        return writeToDatabaseCVETree(JSONData, versionInput);
    }
    ...
    function postCVEHistory(data: any, tree: any, version_number: string): Promise<any> {
        ...
        return ref.once('value').then(function (snapshot) {
            ...
            return buildCVEHistoryTree(result, version_number, tree);
        }).catch(error => { console.log("Error with writing new CVE history tree to db: " + error) });
    }


    function buildCVEHistoryTree(tree: any, versionHistory: string, cveTree: any): any {
        //key is ASB:Version
        ...
        if (tree === null) {
            ...
            for (const ID in cveTree) {
                ...
                promises.push(ref.child(ID).child(tempLongVersionNumber).set(cveTree[ID]).catch(
            ...
            for (const CVE in cveTree) {
                if (!Object.values(cveTree).includes(CVE)) {
                    ...
                    promises.push(ref.child(CVE).child(tempLongVersionNumber).set(cveTree[CVE]).catch(error => { console.log("Error adding in CVE into history tree:" + error) }));
                }
            }
            for (const CVE in tree) {
                ...
                if (JSONData[CVE] !== undefined) {
                    ...
                    if (!hasKey) {
                        ...
                        promises.push(ref.child(CVE).child(longVersionNumberHistory).update(constructedData).catch(error => { console.log("Error adding CVE to history tree:" + error) }));
                    }
                }
            }
        }
    ...
    function writeToDatabaseCVETree(data: any, versionNum: string): Promise<any> {
        ...
        return currentTreeCheck.then(async (versionCheck) => {
            if (versionCheck[0]) { //if not an older version of bulletin being passed in update data
                ...
                return pullFromDatabase(longVersionNumber, versionInput.toString());
            }
    ...
    function pullFromDatabase(version_full: string, version_short: string): Promise<any> {
        const db = admin.database();
        ...
        return refCVE.once('value').then(function (snapshot) {
            ...
            promises.push(sendSPLCVEIDTreeToDB(splCVEIDTree[0], splCVEIDTree[1]));
            const aospVersionASBCVEIDTree = buildAOSPVersionASBCVEIDTree(result);
            promises.push(sendAOSPVersionASBCVEIDTreeToDB(aospVersionASBCVEIDTree[0], aospVersionASBCVEIDTree[1], aospVersionASBCVEIDTree[2]));
    ...
    function sendBulletinSPLTreeToDB(treeToSend: Map<string, Set<any>>): any {
        const db = admin.database();
        ...
        for (const key of treeToSend.keys()) {
            ...
            if (set) {
                ...
                promises.push(refBulletinSPL.child(key).set(array).catch(error => { console.log("Error adding to Bulletin SPL Tree:" + error) }));
            }
        }
    ...
    function sendSPLCVEIDTreeToDB(treeToSend: Map<string, Set<any>>, publishMap: any): any {
        ...
        for (const key of treeToSend.keys()) {
            ...
            if (set) {
                ...
                promises.push(ref.child(key).set(addSet).catch(error => { console.log("Error adding CVE to SPL CVE ID Tree:" + error) }));
            }
        }
    ...
    function sendAOSPVersionASBCVEIDTreeToDB(aospASBMapSend: Map<string, Set<any>>, asbCVEIDMapSend: Map<string, Set<any>>, tree: any): any {
        ...
        for (const key of aospASBMapSend.keys()) {
            ...
            if (asbCVEIDSet) {
                for (const currentEntry of asbCVEIDSet) {
                    ...
                    promises.push(ref.child(tempKey).child(currentEntry).set(array).catch(error => { console.log("Error adding CVE to AOSP Version ASB CVE ID Tree:" + error) }));
                }
            }
        }
    ...
    function sendAOSPVersionCVEIDTreeToDB(treeToSend: Map<string, Set<any>>): any {
        ...
        for (const key of treeToSend.keys()) {
            ...
            if (set) {
                ...
                promises.push(ref.child(key).set(CVEs).catch(error =>
    ...
    function sendBulletinVersionTreeToDB(treeToSend: Map<string, Array<string>>): any {
        ...
        for (const key of treeToSend.keys()) {
            ...
            promises.push(ref.child(key).set(addData).catch(error => { console.log("Error sending Bulletin Version Tree to database" + error) }));
        }
