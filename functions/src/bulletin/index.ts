  ...
  const allSplPromise = bulletinToSplPromise.then(snapshot => {
    ...
    if (splData === null || splData === undefined) {
      res
        .status(404)
        .send(
          'Error: There are no SPLs associated with this bulletin in the database.'
        );
    }
    const promises = [];
...
function getSplsCvesWithAndroidVersion(version: string, res: any) {
  ...
  const aospVerToBulletinPromise = ref
    .orderByKey()
    .equalTo(version)
    .once('value');
  const bulletinSplPromise = aospVerToBulletinPromise.then(snapshot => {
    ...
    if (bulletinData === null || bulletinData === undefined) {
      res
        .status(404)
        .send(
          'Error: There are no SPL and CVE IDs associated with this bulletin in the database.'
        );
    }
    const promises = [];
  ...
  splCvePromise
    ...
    .catch(error => {
      res
        .status(500)
        .send('error getting spls and cveIDs for AndroidVersion: ' + error);
    });
}